...
public class RequestTest {
    ...
    @Test
    public void onReceiveRequestWithoutTokenResponseWithoutToken() throws InterruptedException {
        LogHelper.addLogger(new SystemOutLogger(""));
        lock = new CountDownLatch(1);
        String expectedResponse = "response!";
        ...
        client.send(request, new CoAPHandler() {
            @Override
            public void onMessage(CoAPMessage message, String error) {
                if (error == null && message.getToken() == null) {
                    responseReceived.set(true);
                    responseIsCorrect.set(expectedResponse.equals(message.getPayload().toString()));
                    lock.countDown();
                }
            }
